...
def classifier_wrapper(classifier, classifier_type, test_sample):
    ...
    """
    test_sample = remove_excessive_values(test_sample)
    # print("test sample")
    # print(test_sample)
...
def remove_excessive_columns(feature_matrix):
    #removes columns/features with garbage value like -1
    n,m = feature_matrix.shape
    garbage_columns = []
    for i in range(m):
        if feature_matrix[0][i]==-1:
            garbage_columns.append(i)
    for i in range(len(garbage_columns)):
        for j in range(feature_matrix.shape[1]):
            if feature_matrix[0][j]==-1:
                feature_matrix =  np.delete(feature_matrix,j,1)
                break
    return feature_matrix
def remove_excessive_values(test_sample):
    garbage_index = []
    for i in range(len(test_sample)):
        if test_sample[i]==-1:
            garbage_index.append(i)
    test_sample = np.delete(test_sample,garbage_index)
    return test_sample


def train_logistic_regression(features,c_param):
    feature_matrix, labels = features_to_matrix(features)
    log_reg = sklearn.linear_model.LogisticRegression(C=c_param,random_state=0,solver='lbfgs',)
    feature_matrix = remove_excessive_columns(feature_matrix)


    log_reg.fit(feature_matrix, labels)

    # a = feature_matrix
    # df = pd.DataFrame(a)
    # df['labels']=labels
    # df.to_csv('log_reg.csv', index=False, header=False)
    return log_reg
...
def extract_features_and_train(paths, mid_window, mid_step, short_window,
                               ...
                               compute_beat=False, train_percentage=0.90,feature_list_given=[1,2,3,4,5,6,7,8]):
    """
    ...
    features, class_names, _ = \
        aF.multiple_directory_feature_extraction(paths, mid_window, mid_step,
                                                 ...
                                                 compute_beat=compute_beat,feature_list_given=feature_list_given)

    # features = remove_excessive_columns(features)
    ...
    best_param,Acc = evaluate_classifier(features, class_names, 100, classifier_type,
    ...
    if classifier_type == "svm":
        ...
        classifier = train_logistic_regression(features_norm,best_param)
...
def evaluate_classifier(features, class_names, n_exp, classifier_name, params,
    ...
    for Ci, C in enumerate(params):
        ...
        for e in range(n_exp):
            ...
            if classifier_name == "svm":
                ...
                classifier = train_logistic_regression(f_train,C)
    ...
    if parameter_mode == 0:
        ...
        return params[best_ac_ind],100*ac_all[best_ac_ind]
...
def evaluate_model_for_folders(input_test_folders, model_name, model_type,
                               positive_class, plot=True,feature_list_given=[1,2,3,4,5,6,7,8]):
    """
    ...
    for i, d in enumerate(input_test_folders):
        ...
        for w in wav_file_list:
            c, p, probs_names = file_classification(w, model_name, model_type,feature_list_given=feature_list_given)
            y_pred.append(c)
...
def file_classification(input_file, model_name, model_type,feature_list_given):
    # Load classifier:
    ...
    mid_features, s, _ = \
        aF.mid_feature_extraction(signal, sampling_rate,
                                  ...
                                  round(sampling_rate * short_step),feature_list_given=feature_list_given)
