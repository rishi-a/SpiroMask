...
def feature_extraction(signal, sampling_rate, window, step, deltas=True,feature_list_given=[1,2,3,4,5,6,7,8]):
    """
    ...
    feature_names_all = ["zcr", "energy", "energy_entropy"]
    feature_names_all += ["spectral_centroid", "spectral_spread"]
    feature_names_all.append("spectral_entropy")
    feature_names_all.append("spectral_flux")
    feature_names_all.append("spectral_rolloff")
    feature_names_all += ["mfcc_{0:d}".format(mfcc_i)
                      for mfcc_i in range(1, n_mfcc_feats + 1)]
    feature_names_all += ["chroma_{0:d}".format(chroma_i)
                      for chroma_i in range(1, n_chroma_feats)]
    feature_names_all.append("chroma_std")

    feature_names = []
    for i in feature_list_given:
        if i <9:
            feature_names.append(feature_names_all[i-1])

    if 9 in feature_list_given:
        feature_names += [feature_names_all[i] for i in range(8,21)]
    if 10 in feature_list_given:
        feature_names += [feature_names_all[i] for i in range(21,34)]
        

    features_dict = {}
    for feat in feature_names_all:
        if feat in feature_names:
            features_dict[feat]=True
        else:
            features_dict[feat]=False
    ...
    #required_feature_computations
    is_fft_required= False
    is_mfcc_required=False
    is_chroma_required = False

    for i in range(4,31):
        if i in feature_list_given:
            is_fft_required =True
            break
    if 9 in feature_list_given:
        is_mfcc_required = True

    if 10 in feature_list_given:
        is_chroma_required = True
    ...
    while current_position + window - 1 < number_of_samples:
        ...
        # zero crossing rate
        if features_dict['zcr']:
            feature_vector[0] = zero_crossing_rate(x)

        # short-term energy
        if features_dict['energy']:
            feature_vector[1] = energy(x)

        # short-term entropy of energy
        if features_dict['energy_entropy']:
            feature_vector[2] = energy_entropy(x)

        # sp centroid/spread
        if is_fft_required:
            tmp =  spectral_centroid_spread(fft_magnitude, sampling_rate)
        if  features_dict['spectral_centroid']:       
            feature_vector[3]= tmp[0]

        if features_dict['spectral_spread']:
            feature_vector[4] = tmp[1]


        # spectral entropy
        if features_dict['spectral_entropy']:
            feature_vector[5] = spectral_entropy(fft_magnitude)

        # spectral flux
        if features_dict['spectral_flux']:
            feature_vector[6] = \
                spectral_flux(fft_magnitude,
                              fft_magnitude_previous)

        # spectral rolloff
        if features_dict['spectral_rolloff']:
            feature_vector[7] = \
